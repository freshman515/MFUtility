...
using MFUtility.Mvvm.Wpf.Framework.Managers;

namespace MFUtility.Mvvm.Wpf.Framework.Services {
	public class NavigationService : INavigator {
		...
		public void GoBack(string region) {
			...
			if (regionControl.Content is FrameworkElement oldView &&
				...
				oldVm.OnNavigatingFrom(); // 1. 开始导航前
				oldVm.IsActive = false; // 2. 视图被停用
				oldVm.OnDeactivated(); // 3. 激活状态变化事件
				oldVm.OnNavigatedFrom(); // 4. 导航已完成
			}
