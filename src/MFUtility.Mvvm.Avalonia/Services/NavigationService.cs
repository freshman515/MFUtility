...
using MFUtility.Mvvm.Avalonia.Helpers;
using MFUtility.Mvvm.Avalonia.Interfaces;
using MFUtility.Mvvm.Avalonia.Managers;
...
public class NavigationService : INavigator {
	...
	public void Navigate<TViewModel>(string region, NavigationParameters? parameter = null, Lifetime lifetime = Lifetime.Singleton) {
		Navigate(typeof(TViewModel), region, parameter, lifetime);
	}
	...
	public void Navigate(Type viewModelType, string region, object? parameter = null, Lifetime lifetime = Lifetime.Singleton) {
		...
		// =====================================================
		if (regionControl.Content is Control currentView &&
		...
		// 1. 旧 VM：即将离开（可阻止导航）
		// ===============================
		if (regionControl.Content is Control oldView &&
	...
	public object? GetCurrent(string region) {
		var regionControl = RegionManager.GetRegion(region);
		return regionControl.Content is Control view
			? view.DataContext
			: null;
	}
	// ================================
