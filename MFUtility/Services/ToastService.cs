...
public static class ToastService {
	...
	public static void Show(
		...
		Application.Current.Dispatcher.Invoke(async () => {
			...
			var (left, top) = CalculatePosition(position, width, height, mode, list);
			toast.Left = left;
			toast.Top = top;
	...
	private static (double left, double top) CalculatePosition(
		NotifycationPosition position, double width, double height,
		NotificationMode mode, List<ToastWindow> list) {
		...
		switch (position) {
			...
			case NotifycationPosition.TopRight:
				left = screenW - width - 20; top = 20; break;
			case NotifycationPosition.BottomLeft:
				left = 20; top = screenH - height - 20; break;
			case NotifycationPosition.BottomRight:
				left = screenW - width - 20; top = screenH - height - 20; break;
			case NotifycationPosition.TopCenter:
				left = (screenW - width) / 2; top = 40; break;
			case NotifycationPosition.BottomCenter:
				left = (screenW - width) / 2; top = screenH - height - 40; break;
			case NotifycationPosition.Center:
			default:
				left = (screenW - width) / 2; top = (screenH - height) / 2; break;
		}

		// ✅ 堆叠逻辑：当前队列内独立处理
		if (mode == NotificationMode.Stack && list.Count > 0) {
			var last = list.Last();
			if (IsTopPosition(position))
				top = last.Top + (height + Gap); // 向下叠
			else
				top = last.Top - (height + Gap); // 向上叠
		}
